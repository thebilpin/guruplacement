...
import { ComplianceRecord, ComplianceDocument, AuditRecord } from '../src/lib/schemas/compliance';
...
async function seedComplianceData() {
  ...
  try {
    ...
    const sampleComplianceRecords: Omit<ComplianceRecord, 'id'>[] = [
      {
    ...
    for (const audit of sampleAuditRecords) {
      await collections.activityLogs().add(audit);
    }
