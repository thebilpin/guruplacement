...
export async function GET(request: NextRequest) {
  try {
    console.log('Student compliance API called');
    ...
    if (!dataInitialized) {
      console.log('Initializing student compliance data...');
      initializeData();
    ...
    switch (action) {
      ...
      case 'students':
        console.log('Returning students list');
      ...
      case 'student':
        ...
        console.log('Returning student details for:', studentId);
        return getStudentDetails(studentId);
      case 'alerts':
        console.log('Returning alerts');
        return getAlerts();
      case 'requirements':
        console.log('Returning compliance requirements');
        return getComplianceRequirements();
      default:
        console.log('Default: returning dashboard data');
        return getDashboardData();
    }
  } catch (error) {
...
function getDashboardData(): NextResponse {
  try {
    console.log('Calculating student compliance stats...');
    ...
    const recentAlerts = alertsData
      ...
      .slice(0, 5);
    console.log('Recent alerts processed:', recentAlerts.length);
